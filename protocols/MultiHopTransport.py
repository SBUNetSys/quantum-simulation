...
class TransportOperation:
    def __init__(self, source_node, source_mem_pos, target_node, target_mem_pos, op_key, measurement_results=None):
...
class Transportation(NodeProtocol):
    ...
    Each node will only forward the measurement result to next hop
    ...
    def __init__(self, node,
        ...
        # key: entangled_node(in_node), value = {mem_pos: measurement_result []}
        ...
        # pending measurement result i.e transport start
        self.pending_need_measurement_queue = {}  # key(entangle_node, mem_pos): TransportApplyCorrectionMessage
    ...
    def run(self):
        ...
        :return:
        """
        transport_signal = self.await_signal(self.cc_message_handler, signal_label=MessageType.MULTI_HOP_RESULT)

        self.logger.info(f"MultiHop Transport {self.name} -> Start transport protocol", color="cyan")
        self.start_time = sim_time()
        while True:
            ...
            if expr.first_term.value:
                ...
                for event in expr.first_term.triggered_events:
                    ...
                    try:
                        ...
                        self.logger.info(f"MultiHop Transport {self.name} -> Failed to parse signal\n"
                    ...
                    if type(result) is SwapEntangledSuccess:
                        self.logger.info(f"MultiHop Transport {self.name} -> Qubit Ready signal from e2e {entangle_node}\n"
                    ...
                    if type(mem_pos) == list:
                        for pos in mem_pos:
                            ...
                            self.logger.info(
                                f"MultiHop Transport {self.name} Received Entangled Qubit\n"
                ...
                for event in expr.second_term.triggered_events:
                    ...
                    if ready_signal.label == MessageType.MULTI_HOP_RESULT:
                        ...
                        self.logger.info(f"MultiHop Transport {self.name} -> Transport measurement result\n"
    ...
    def check_transport_ready(self):
        ...
        if self.is_source_node and self.sent_qubit_count < self.transmitting_qubit_size:
            ...
            for _ in range(min(entangled_qubits_size, qubits_need_size)):
                ...
                self.logger.info(f"MultiHop Transport {self.name} sending qubits\n"
        ...
        for key in self.pending_transmission_qubits.keys():
            if len(self.pending_transmission_qubits[key]) > 0:
                ...
                for _ in range(min(need_transmission_qubit_size, ready_qubit_size)):
                    ...
                    op = TransportOperation(key, source_mem_pos, self.entangled_node,
                                            target_mem_pos,op_key, measurement_results)
                    transmission_ops.append(op)
        ...
        # now we need to check if we have pending measurement result if we do, we need process them
        process_correction = self.process_transmit_queue()
        if len(process_correction) > 0:
            yield from self.handle_multi_hop_results_batch(process_correction)

    def process_transmit_queue(self):
        remove_message = []
        correction_message = []
        for key in self.pending_need_measurement_queue.keys():
            node = key[0]
            mem_pos = key[1]
            if node in self.entangled_qubits and mem_pos in self.entangled_qubits[node]:
                message = self.pending_need_measurement_queue[key]
                correction_message.append(message)
                remove_message.append(key)
        for key in remove_message:
            self.pending_need_measurement_queue.pop(key)
    ...
    def start_transport_measurement(self, transmission_ops: list):
        ...
        for op in transmission_ops:
            if self.is_source_node:
                ...
                self.logger.info(
                    f"MultiHop Transport {self.name} Start Teleporting Qubit as Sender\n"
                ...
                self.logger.info(
                    f"MultiHop Transport {self.name} Start Teleporting Qubit Measurement as Middle Node\n"
                    f"Operation: {op.operation_key}\n"
            ...
            message = MultiHopTransportMeasurementMessage(
            ...
            # append the measurement results with previous info
            message.add_measurement_result(m1, m2)
            measure_result_messages[op.target_node].append(message)

        # now send the measurement to next hop to measurement result
        ...
        for target_node in measure_result_messages.keys():
            self.cc_message_handler.send_message(MessageType.MULTI_HOP_RESULT,
                                                 target_node,
                                                 ClassicalMessage(
    ...
    def handle_multi_hop_results_batch(self, messages: list):
        """
        handle measurement result from source node to finish the teleport operation.
        ...
        for message in messages:
            ...
            if message.target_node not in self.entangled_qubits or \
                ...
                self.logger.info(
                    f"MultiHop Transport {self.name} received measurement result, node is not ready\n"
                ...
                # append to pending measurement result q
                self.pending_need_measurement_queue[(message.target_node, message.target_memo_pos)].append(message)
                continue
            self.logger.info(
                f"MultiHop Transport {self.name} received measurement result\n"
                f"Operation: {message.operation_key}\n"
                f"Source node: {message.source_node}\n"
                f"Target node: {message.target_node}\n"
                f"Target memo pos: {message.target_memo_pos}\n", color="yellow"
            ...
            self.pending_transmission_qubits[message.target_node][message.target_memo_pos] = message.measurement_results
            if self.is_destination_node:
                # apply all the correction based on the measurements
    ...
    def reset(self):
        ...
        self.logger.info(f"MultiHop Transport {self.name} -> Resetting Transport, sending message", color="red")
        ...
        self.sent_qubit_count = 0
        self.pending_need_measurement_queue = {}  # key(entangle_node, mem_pos): TransportRequestMessage
        self.memory_mapping = {}
        self.final_result = defaultdict(dict)
        super().reset()

    def stop(self):
        self.logger.info(f"MultiHop Transport {self.name} -> Stopping Transport, sending message", color="red")
        super().stop()