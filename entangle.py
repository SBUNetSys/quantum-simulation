...
class DirectionalEntanglementProtolMultiMem(NodeProtocol):
    ...
    def run(self):
        ...
        print(f"Entangle -> Node {self.node.name} has\n"
        ...
        while True:
            ...
            #     print(f"Entangle -> Got start expression: {self.start_expression.value}, starting... {self.node.name}")
            #     yield re_entangle
            ...
            #         # print(f"Entangle -> {self.node.name} has entangled all pairs for both neighbours.")
            ...
            #         # print(f"Entangle -> {self.node.name} has entangled all pairs for left neighbour.")
            #         # pass
            #         yield re_entangle
            #         pass
            #         # pass
            ...
            #         # print(f"Entangle -> {self.node.name} has entangled all pairs for right neighbour.")
            #         # pass
            #         yield re_entangle
            #         pass
            #         # pass
            if self._qmem_input_port_left is not None and self._qmem_input_port_right is not None:
                print(f"Entangle -> Node {self.node.name} had both left and right neighbours.")
                if self._is_source and len(self.right_aval_mem_positions) > 0:
                    ...
                    qsource.trigger()
                    print(f"Entangle -> Node {self.node.name} generating qubit with "
                    ...
                    print(f"Entangle -> Node {self.node.name} initial fidelity: {initial_fidelity}")
                ...
                print(f"Entangle -> Node {self.node.name} had only left neighbour.")
                expr = yield (self.await_port_input(self._qmem_input_port_left) |
                              re_entangle)
                if expr.triggered_events:  # Ensure there are triggered events to process
                    for event in expr.triggered_events:
                ...
                print(f"Entangle -> Node {self.node.name} had only right neighbour {self.right}.")
                if self._is_source and len(self.right_aval_mem_positions) > 0:
                    qsource = self.node.subcomponents[self._qsource_name]
                    ...
                    self._right_qport.tx_input(qubit_1)
                    # send the qubit to the right neighbour
                    print(f"Entangle -> Node {self.node.name} sending qubit to {self.right}")
                    self.node.ports["qout0"].tx_output(qubit_2)
                    # yield self.await_timer(duration=10000.0)
                    # time for the qubit to be sent
    ...
    def handle_re_entangle(self, expr):
        ...
        if expr.value:
            ...
            print(f"Entangle -> Node {self.node.name} received signal: {ready_signal.result}")
    ...
    def handle_entangle_left(self):
        ...
        print(f"Entangle -> Node {self.node.name} Received qubit from left neighbour {self.left},"
        ...
        print(f"Entangle -> Node {self.node.name} entangled left pairs: {self.left_entangled_pairs}\n"
    ...
    def handle_entangle_right(self):
        ...
        print(f"Entangle -> Node {self.node.name} Received qubit from QSource,"
        ...
        print(f"Entangle -> Node {self.node.name} entangled right pairs: {self.right_entangled_pairs}")
...
class GenEntanglement(NodeProtocol):
    ...
    def run(self):
        ...
        while True:
            ...
            if self._qmem_input_port is not None:
                print(f"Entangle -> Node {self.node.name} had entangle node {self.entangle_node}.")
                ...
                if self._is_source and len(self.aval_mem_postions) > 0:
                    ...
                    qsource.trigger()
                    print(f"Entangle -> Node {self.node.name} generating qubit with "
                          f"entangled {self.entangled_pairs}")
                    # wait for qsource to generate qubit and make sure both we give enough
                    ...
                    # send the qubit to the right neighbour
                    print(f"Entangle -> Node {self.node.name} sending qubit to {self.entangle_node}")
                    self.node.ports["qout0"].tx_output(qubit_2)
                    # yield self.await_timer(duration=10000.0)
                    # time for the qubit to be sent
    ...
    def handle_entangle(self, init_fidelity):
        ...
        print(f"Entangle -> Node {self.node.name} Received qubit from entangle node,"
        ...
        print(f"Entangle -> Node {self.node.name} entangled pairs: {self.entangled_pairs}\n"
    ...
    def stop(self):
        ...
        if self.used_mem_positions:
            ...
            for i in self.used_mem_positions[1:]:
                self._qmemory.mem_positions[i].in_use = False
