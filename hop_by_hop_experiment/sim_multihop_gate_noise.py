"""
Hop-by-hop transport experiment with gate noise, end-of-path correction.

Differences from sim_transport_gate_noise.py:
  - Uses protocols/MultiHopTransportGateNoise.py: intermediate nodes forward Bell
    measurement results without applying corrections; only the destination applies
    the XOR'd correction once.
  - Gate operations still run through gate_processor for physical delay and noise.
  - No verification.
  - Awaits MULTI_HOP_FINISHED.
"""
import gc
import json
import os
...
from protocols.MultiHopTransportGateNoise import Transportation
from utils.SignalMessages import ProtocolFinishedSignalMessage


class MultiHopGateNoiseExample(LocalProtocol):
    """
    Outer protocol for multi-hop transport with gate noise, end-of-path correction.
    Awaits MULTI_HOP_FINISHED from the destination's transport subprotocol.
    """
...
def example_sim_run(nodes, num_runs, memory_depolar_rate, node_distance,
    ...
    example = MultiHopGateNoiseExample(
        network_nodes=nodes,
        num_runs=num_runs,
...
def run_multihop_gate_noise_worker(queue, distance, target_fid, depolar_rate, node_count,
                                   with_purify, run_index, data_save_path, raw_data_save_path,
                                   gate_depolar_rate, gate_duration_ns,
                                   cnot_depolar_rate, cnot_duration_ns):
    try:
        node_data = {}
        node_data_raw = {}
        nodes_list = [f"Node_{i}" for i in range(node_count)]
        network = setup_network_parallel_with_gate_noise(
            nodes_list, "hop-by-hop-gate-noise-multihop",
            memory_capacity=101,
...
def run_multihop_gate_noise_multiprocess(distance, target_fid, depolar_rate, node_count,
    ...
    data_save_path = os.path.join(
        save_path,
        f"multihop_gate_noise_{node_count}_nodes_{distance}km@{depolar_rate}hz_"
    ...
    while success_run < total_runs:
        ...
        queue = Queue()
        process = Process(
            target=run_multihop_gate_noise_worker,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Multi-hop transport with gate noise, end-of-path correction')
    parser.add_argument('--distance', type=float, default=1.0, help='Node distance in km')
    parser.add_argument('--target-fid', type=float, default=0.99,
                        help='Target fidelity for purification')
    parser.add_argument('--depolar-rate', type=int, default=6000,
                        help='Memory depolar rate (Hz)')
    ...
    parser.add_argument('--save-path', type=str, default='./gate_noise_results/',
    ...
    print(
        f"Running multi-hop gate noise transport (end-of-path correction):"
    ...
    run_multihop_gate_noise_multiprocess(
