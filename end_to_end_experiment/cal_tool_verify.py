...
if __name__ == '__main__':
    ...
    with open("./transportation_results/e2e_3nodes_throughput_1km_verify_raw.json", "r") as file:
        data = json.load(file)
    g_7_counts = []
    g_9_counts = []
    g_7_global_count = 0
    ...
    print(f"e2e 3 Nodes 1km Throughput Count: {np.mean(all_count)}")
    print(f"e2e 3 Nodes 1km Throughput > 0.7: {np.mean(g_7_counts)}")
    print(f"e2e 3 Nodes 1km Throughput > 0.9: {np.mean(g_9_counts)}")
    print(f"e2e 3 Nodes 1km Global > 0.7: {g_7_global_count / total_count}")
    print(f"e2e 3 Nodes 1km Global > 0.9: {g_9_global_count / total_count}\n")


    with open("./transportation_results/e2e_3nodes_1_qubit_verification_0.5km_raw.json", "r") as file:
        data = json.load(file)
    fid_data = data["teleport_fids"]
    g_7_count = 0
    g_9_count = 0
    ...
    print(f"3 Nodes 0.5 Km Verify > 0.7: {g_7_count/ len(all_fid)}")
    print(f"4 Nodes 0.5 Km Verify > 0.9: {g_9_count/ len(all_fid)}")
    print(f"average duration: {np.mean(d_data)}")
    print(f"average fid: {np.mean(all_fid)}\n")